...
class Smartg4controlPacket {
    ...
    public static int ushort(byte h, byte l) {
    ...
    public static Smartg4controlPacket parse(byte[] data, int length) {
        // 4 bytes for IP + 10 bytes for HDLMIRACLE + 13 bytes min packet length == 27
        // String s = new String(data, 4, 10);
        // System.out.println(s);
        if (length < 27 || !(new String(data, 4, 10).equals("SMARTCLOUD"))) {
            return null;
        }
        ...
        Smartg4controlPacket packet = new Smartg4controlPacket();
        packet.sourceSubnetID = data[offset];
        packet.sourceDevideID = data[offset + 1];
        if (packet.sourceDevideID < 0) {
            packet.sourceDevideID = 256 + packet.sourceDevideID;
        }
        ...
        packet.targetDevideID = data[offset + 1];
        if (packet.targetDevideID < 0) {
            packet.targetDevideID = 256 + packet.targetDevideID;
        }
    ...
    public String toString() {
        String ret = "[" + Integer.toHexString(command) + "]" + "[" + sourceAddress + "-" + targetAddress + "-cmd:"
                + Integer.toHexString(command) + "-deviceCode:" + sourceDevice + "]  [ SOURCE = sub:" + sourceSubnetID
                + " devId:" + sourceDevideID + "] [ TARGET = sub:" + targetSubnetID + " devId:" + targetDevideID
                + "] + DATA[";
    ...
    public void setTargetAddress(int a) {
        targetAddress = a & 0xffff;
    }

    public void setSourceAddress(int a) {
        sourceAddress = a & 0xffff;
    }
    ...
    public byte[] getBytes() {
        ...
        byte[] magic = "SMARTCLOUD".getBytes();
        ...
        p[i++] = (byte) (sourceAddress >> 8); // src subnet
        p[i++] = (byte) sourceAddress; // src device id
        p[i++] = (byte) (sourceDevice >> 8); // src subnet
        p[i++] = (byte) sourceDevice; // src device id
        p[i++] = (byte) (command >> 8); // cmd id
        p[i++] = (byte) command; // cmd id
        p[i++] = (byte) targetSubnetID; // tgt subnet
        p[i++] = (byte) targetDevideID; // tgt device id
    ...
    public int getSourceDevideID() {
        return sourceDevideID;
    }

    public void setSourceDevideID(int sourceDevideID) {
        this.sourceDevideID = sourceDevideID;
    }

    public int getTargetSubnetID() {
        return targetSubnetID;
    }

    public void setTargetSubnetID(int targetSubnetID) {
        this.targetSubnetID = targetSubnetID;
    }

    public int getTargetDevideID() {
        return targetDevideID;
    }

    public void setTargetDevideID(int targetDevideID) {
        this.targetDevideID = targetDevideID;
    }
}
